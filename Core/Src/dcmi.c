...
volatile uint8_t DCMI_FrameState = 0;   /* 帧完成标志 — 帧数据传输完成时由 HAL_DCMI_FrameEventCallback() 中断回调置 1 */
volatile uint8_t OV2640_FPS;            /* 帧率 (FPS) */
/* USER CODE END 0 */
...
void HAL_DCMI_MspInit(DCMI_HandleTypeDef* dcmiHandle)
{
  ...
  /* USER CODE BEGIN DCMI_MspInit 1 */
    /* PWDN 引脚初始化 — 高电平进入省电模式（摄像头停止输出，最大限度省电）*/
    OV2640_PWDN_ON;
    ...
    HAL_GPIO_Init(OV2640_PWDN_PORT, &GPIO_InitStruct);
 ...
 *  功能描述: 初始化 SCCB + DCMI + DMA 并配置 OV2640
 ******************************************************************************************/
int8_t DCMI_OV2640_Init(void)
{
    uint16_t Device_ID;

    SCCB_GPIO_Config();                    /* SCCB 引脚初始化 */
    OV2640_Reset();                        /* 硬件复位 */
    Device_ID = OV2640_ReadID();           /* 读取摄像头 ID */

    if ((Device_ID == 0x2640) || (Device_ID == 0x2642))
    {
        printf("OV2640 OK, ID: 0x%X\r\n", Device_ID);

        /* 默认加载 SVGA 模式 (800×600, 最高 30fps) */
        OV2640_Config(OV2640_SVGA_Config);
        // OV2640_Config(OV2640_UXGA_Config);  /* UXGA 模式 (1600×1200, 最高 15fps) */
...
void OV2640_DMA_Transmit_Continuous(uint32_t DMA_Buffer,uint32_t DMA_BufferSize)
{
   ...
   DCMI_FrameState = 0;  /* 清除帧完成标志，等待新帧 */
...
{
   hdma_dcmi.Init.Mode  = DMA_NORMAL;    /* 普通模式 */
...
{
   HAL_DCMI_Resume(&hdcmi);                    /* 恢复 DCMI 捕获 */
}
...
*				 4. ���������ˮƽ�ʹ�ֱƫ�ƣ������û�����вü�
*****************************************************************************************************************************************/
int8_t OV2640_DCMI_Crop(uint16_t Display_XSize, uint16_t Display_YSize, uint16_t Sensor_XSize, uint16_t Sensor_YSize )
{
	...
	DCMI_X_Offset = Sensor_XSize - Display_XSize;

	DCMI_Y_Offset = (Sensor_YSize - Display_YSize) / 2 - 1;
 ...
 *  功能描述: 帧回调函数 — 每完成一帧数据，产生中断回调
 *  说   明: 每次传输完一帧数据后置位帧完成标志，并统计帧率
 ******************************************************************************************/
void HAL_DCMI_FrameEventCallback(DCMI_HandleTypeDef *hdcmi)
{
    static uint32_t DCMI_Tick = 0;          /* 保存当前时间戳 */
    static uint8_t  DCMI_Frame_Count = 0;   /* 帧计数器 */

    if (HAL_GetTick() - DCMI_Tick >= 1000)   /* 每 1s 统计一次帧率 */
    {
        DCMI_Tick = HAL_GetTick();
        OV2640_FPS = DCMI_Frame_Count;
        DCMI_Frame_Count = 0;
    }
    DCMI_Frame_Count++;
    DCMI_FrameState = 1;  /* 帧完成标志置 1（由消费者清零）*/
}

/*****************************************************************************************
 *  函 数 名: HAL_DCMI_ErrorCallback
 *  功能描述: DCMI 错误回调
 *  说   明: 检测 DMA 溢出 / FIFO 溢出 / 过载等错误，当前仅预留接口
 ******************************************************************************************/
void HAL_DCMI_ErrorCallback(DCMI_HandleTypeDef *hdcmi)
{
    /* 预留：可根据 HAL_DCMI_GetError() 诊断具体错误类型 */
    (void)hdcmi;
}
/* USER CODE END 1 */
